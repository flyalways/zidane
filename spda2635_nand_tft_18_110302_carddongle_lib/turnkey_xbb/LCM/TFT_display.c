...
void TFT_ShowFMFrequency(void)
{
	...
	for(i=0;i<4;i++)
	{
		...
		LCM_set_view(TFT_IconDispDir,FM_Freq_X_Pos+FM_Freq_H_Size*i+j+i,FM_Freq_Y_Pos,FM_Freq_H_Size,FM_Freq_V_Size);
...
#else
		LCM_set_view(TFT_IconDispDir,BitRate_X_Pos+(BitRate_H_Size+2)*i,BitRate_Y_Pos,BitRate_H_Size,BitRate_V_Size);
#endif
...
void TFT_ShowError(void)
{
	LCM_set_view(TFT_IconDispDir,Error_X_Pos,Error_Y_Pos,Error_H_Size,Error_V_Size);
	LCD_DisplayIcon(ErrorID,0);
}
