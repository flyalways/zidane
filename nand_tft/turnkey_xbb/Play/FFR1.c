...
void UI_fastFFFR_Play(U8 XFFFR)
{
	...
	if((gs_System_State.c_Phase != TASK_PHASE_FASTFWD)&&(gs_System_State.c_Phase!=TASK_PHASE_FASTREV))
	{
		...
		{    
		    ...
		    gw_CurrentSec = (*Music_ReadTime[gc_Play_FileType])();//(JC)Time reset after fffr
			switch(gc_Play_FileType)
			{
				case 0x00: //MP3
...
void play_fffr_end()
{
    ...
    if(tdw_datapoint>0)
    {
		...
		gb_fffr_end=1;//(JC)H1027 for FFFR end WMA current time display
	}
}
