...
extern  xdata	U8	gc_ShowPingpuTimer;
extern	xdata	U8  gb_ShowPingpu;
...
{
    if (gs_DSP_GLOBAL_RAM.sw_Volume<62)
    {
...
/*	if((gs_System_State.c_Phase==TASK_PHASE_PLAYACT))//&&(gb_ShowPingpu==1))
	{
		playstate=1;				//播放状态
	}
	else
...
*/	
	if((gb_ShowPingpu && (gs_System_State.c_Phase==TASK_PHASE_PLAYACT))||(gb_TFT_refresh ==1))
	{
		TFT_ShowPingpu();
		TFT_ShowPlayRatioBar();
		gc_ShowPingpuTimer=5;
...
#ifdef TFT_18H     //sunzhk add 090617
    // 簈Ρ丁
	...
	while(1)
	{
        switch(gc_PhaseInx)
        {
            ...
            case C_PlayVolUp:
					 ...
					 if(gc_TuneVolFreqStatus==0||gc_TuneVolFreqStatus==1)
					{
                		play_volup();
					}
