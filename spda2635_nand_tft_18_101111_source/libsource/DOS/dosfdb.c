...
U8 Find_Fdb(SearchFdb *p_mp)
{
	...
	U8	tc_PlayMode;
	U8	tc_ExtNum;
	U16 tw_FdbOffset,tw_Temp_FdbOffset,tw_FileLongName,tw_ByteOffset ,tw_PrevFDB_Offset;
	U16	tw_DirIndex=0,tw_DirIndex1=0;
	U32 tdw_SectorAdd,tdw_StartCluster,tdw_S_ClusterNum,tdw_FDBClusterNum;
	U32 tdw_Find_ClusterSize,tdw_TheEndCluster,tdw_StartCluster1,tdw_StartCluster3, tdw_PrevFDB_Cluster, tdw_PrevFDB_LogAdd;
	...
	{ //find in one	dir
		if((gc_CurrentCard==2) && (SD_Detect==1))  //chiayen0919 add
		{
			return 1;
		}
		...
		{//find	in one cluster 	
			...
			for(tw_FdbOffset=tw_Temp_FdbOffset;tw_FdbOffset<K_DOS_SectorSize;tw_FdbOffset+=32)
			{//Find in one sector			
				tc_PlayMode=0;				
				tc_RecFlag=1;
				tw_DirIndex++;
				...
				{				
					if((p_mp->c_Search_Mode==K_FIND_FREE_FDB) || (p_mp->c_Search_Mode==K_FIND_LAST_FDB))//lzp add for create long name 05/02/19  
					{
						tc_sts=SUCCESS;
					}
					else
...
MatchEXTName:
									...
									{//compare the last longfdb
										if(gc_UserDataBuf[tw_FdbOffset+Unicode_Offset[(*(p_mp->pc_LongFileName+1))+tw_ByteOffset]]!=p_mp->pc_LongFileName[tw_FileLongName-tw_ByteOffset-1])  //last longfdb is not equal
										{
											...
											{//if current char is lower letter,try to compare after converting to upper letter
												if (gc_UserDataBuf[tw_FdbOffset+Unicode_Offset[(*(p_mp->pc_LongFileName+1))+tw_ByteOffset]]!=(p_mp->pc_LongFileName[tw_FileLongName-tw_ByteOffset-1] + 0x20))
												{
													break;
												}
											}
						...
						{//not the	"last" longfdb
							...
							{//compare the longfdb which is not the last
								if(gc_UserDataBuf[tw_FdbOffset+Unicode_Offset[tw_ByteOffset]]!=p_mp->pc_LongFileName[tw_FileLongName-tw_ByteOffset-1])
								{
									if ((gc_UserDataBuf[tw_FdbOffset+Unicode_Offset[tw_ByteOffset]] >= 'a') && (gc_UserDataBuf[tw_FdbOffset+Unicode_Offset[tw_ByteOffset]] <= 'z'))
									{//if current char is lower letter,try to compare after converting to upper letter
										if (gc_UserDataBuf[tw_FdbOffset+Unicode_Offset[tw_ByteOffset]]!=(p_mp->pc_LongFileName[tw_FileLongName-tw_ByteOffset-1] + 0x20))
										{
											break;
										}
									}
				...
				{//short_fdb
					...
					{//is dir
						...
						else if(gb_CountDirFlag && (p_mp->c_Search_Mode != K_SPECIFIC_STARTCLUSTER)) //Ching 100514
						{
							if(gc_UserDataBuf[tw_FdbOffset]!=0x2e) //. & ..
								gw_DirTotalNumber++; 
						}
...
NextFIND:   //Ching 080805
						...
						else if (p_mp->c_Search_Mode == K_SPECIFIC_LONG_FILENAME)
						{
							...
							if (tw_FdbOffset == 0x200)
							{
								...
								DOS_Read_LogicSector((tdw_SectorAdd+tc_SectorOffset),1);//tangwei add 090314 for RTC
							}
							else
							{
								p_mp->dw_LongFDB_LogAdd1=tdw_FDB_LogAdd1;//add for DRM
