...
extern  xdata	U8 media_changetimes;
...
//		dbprintf("=jpeg init=\n");
        ...
        { 
           tc_media_changetimes=3; 
        } 
        else if((gb_SD_Exist==1) || (gb_Host_Exist==1)) 
        { 
                tc_media_changetimes=2; 
        } 
        else   
        {                 
                 tc_media_changetimes=1; 
        }  
		...
		{
			if(media_changetimes==tc_media_changetimes)
		 	{
...
void JpegMenu_Disp()
{
	if(gc_TuneVolFreqStatus==31)
	{
		...
		if(gc_DispWallpaper!=32)
		{
			...
			gc_DispWallpaper=32;
			gc_LCMDispIndex=0;
			gc_LCMDispIndexBak=0x00;			//home
			gc_PhaseInx=0;
			gc_DispPlayMenuAct =0x03;				
		}
